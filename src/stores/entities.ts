...
import type { Entity, EntityProposal, EntitiesState } from '../components/types';
...
export const useEntitiesStore = defineStore('entities', () => {
  ...
  const fetchEntities = async (filters: { entity_type?: string; action_zone?: string; is_approved?: boolean } = {}): Promise<void> => {
    loading.value = true;
  ...
  const addEntity = async (entityData: Omit<Entity, 'id' | 'is_approved' | 'created_at' | 'updated_at'>): Promise<boolean> => {
  ...
  const proposeEntity = async (entityData: Omit<Entity, 'id' | 'is_approved' | 'created_at' | 'updated_at'>): Promise<boolean> => {
  ...
  const approveProposal = async (proposalId: string, entityData: Omit<Entity, 'id' | 'created_at' | 'updated_at'>): Promise<boolean> => {
